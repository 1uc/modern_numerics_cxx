...
//
// by value: the argument is copied an the function receives a copy of the
...
// by reference: a 'const reference' is passed to the function. This is just
//           like a reference, but the function is not permitted to modify the
//           argument.
//
...
// The motivation for using a (non-) const reference are clear: either you
...
// argument by value.  Sofar nothing gained, but what if the place where the
...
public:
  ...
  // we can use the move construtor to initialize `x_`.
...
int main() {
  ...
  std::vector<double> y{0.0, 1.0, 2.0}; // a spare just in case.
  ...
  // You're clearly stating that you're handing over ownership of `x` to
  // `foo_by_val`.
  foo_by_val(std::move(x));
  // From now on `x` must not be used again. Tricky.

  // A more realistic scenario is when creating a new object.
  auto foo = Foo(std::move(y)); // We don't need `y` anymore and `Foo` can take
                                // over the ownership of `x`.
